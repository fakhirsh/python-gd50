...
'''
    Assuming the bricks are all 32x16, we can generate them by
    dividing the atlas into a grid of 6 bricks by 4 bricks. This 
    is basically the upper portion of the sprite sheet.
'''
def GenerateQuadsBricks(sheet_width, sheet_height):
    # Adjust the indices if necessary based on your sprite sheet layout
    # Python's list indexing starts at 0, and the slice end index is exclusive
    # Remember lot of quads do not contain bricks, so we ignore them.
    return GenerateQuads(sheet_width, sheet_height, 32, 16)[:21]
