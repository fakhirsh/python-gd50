...
def load():
    ...
    global window, virtual_screen, clock, matchImg, quads, tiles
...
#---------------------------------------------------------
    # Load images
    matchImg = pygame.image.load('assets/images/match3.png')
    quads = GenerateQuads(matchImg.get_width(), matchImg.get_height(), 32, 32)
    refreshTiles()
...
def render(dt):
    ...
    for y in range(288//32+1):
        for x in range(512//32+1):
            virtual_screen.blit(matchImg, ((x - 1) * 32, (y - 1) * 32), tiles[y][x])
...
def refreshTiles():
    ...
    for y in range(288 // 32+1):
        ...
        for x in range(512 // 32+1):
            tiles[y].append(quads[random.randint(0, len(quads) - 1)])
