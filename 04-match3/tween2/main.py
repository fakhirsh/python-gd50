...
#---------------------------------------------------------
    ...
    # set up x position tweens for each bird:
    for bird in birds:
        # create a tween that will move the bird across the screen over 5 seconds
        # and then loop back to the beginning
        TweenManager.create_tween(bird, 'x', 0, VIRTUAL_WIDTH - birdImg.get_width(), bird['rate'], linear)
        TweenManager.create_tween(bird, 'y', bird['y'], bird['end_y'], bird['rate'], linear)
        TweenManager.create_tween(bird, 'opacity', 0, 255, bird['rate'], linear)
...
def render(dt):
    ...
    for bird in birds:
        temp_surface = birdImg.copy()
        temp_surface.set_alpha(bird['opacity'])
        virtual_screen.blit(temp_surface, (bird['x'], bird['y']))
